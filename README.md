...
- **跨会话记忆**：复盘按需把环境/项目事实、约定教训写入 `~/.dsh/memory/MEMORY.md`，把用户画像与偏好写入 `USER.md`（对齐 Hermes 的记忆分层）；记忆全文随运行时上下文注入每个对话，写入或手改后下一个模型步即生效。按需产出——多数复盘不产生记忆，不为写而写
...
5. 记忆条目在面板查看用量；增删改直接编辑 `~/.dsh/memory/MEMORY.md` / `USER.md`（一行一条、`§ ` 开头），下一个模型步自动生效
